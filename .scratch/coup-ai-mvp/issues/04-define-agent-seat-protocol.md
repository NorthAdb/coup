...
Agent 必须返回结构化游戏命令；权威路径仍不索取、不存储、不展示思维链或原始 transcript。可选的一句决策说明是本局展示旁路（见 ADR-0001），不得进入领域事件或回放：
...
- 2026-08-04 grilling：在不打开思维链/transcript 的前提下，允许本局内存中的可选决策说明旁路；权威 `SeatDecision` 形状不变。详见 ADR-0001 与规格 US-42/56/57。
