...
- **完整规则引擎** `packages/brass-domain`（纯函数、无 IO）：6 产业 45 块瓦片/人、22 个可建地点（含 2 农场酒厂）、39 条运河/铁路连线、5 商人位 9 板、煤/铁市场与收入轨、Build/Network/Develop/Sell/Loan/Scout/Pass 七种行动全量校验（每行动弃 1 张手牌、煤最近优先、双轨啤酒、覆盖规则、商人啤酒奖励、时代切换与两段计分）。
...
- **独立视觉**：入口 `/brass`，工业时代黄铜/羊皮纸主题，SVG 版图（实体版图地理精调坐标、石板建造地块、**每城一座剪影**——Stoke 瓶窑/Dudley 城堡/Coalbrookdale 高炉/Coventry 双塔教堂等九类原型、产业瓦片正反面外观与产业徽标、建造/铺路幽灵预览、运河水系/铁路枕木、煤立方/铁锭/啤酒桶立体 token、罗盘与铭牌、纸纹暗角）+ 局内动效（落子弹入、翻面闪烁、可铺连线蚂蚁线、可建地点呼吸辉光、日志滑入；尊重 prefers-reduced-motion）+ 行动向导（自动推导合法目标与资源来源，卖货模式高亮可卖地点、覆盖建造目标选择、缺额拆板面板）、市场格价条与当前买/卖价、竖版手牌卡（类型色带 + 产业色字块）、玩家面板数据格子与带字面板产业条、中文日志（玩家色点）。门户首页 `/` 选择游戏。
